...
class BitFlipEnvironment:
    def __init__(self, bits):
        ...
        self.frames = []  # Store raw state representations instead of figures
        self.reset()
    ...
    def step(self, action):
        self.state[action] = 1 - self.state[action]  # Flip bit at action index
        reward, done = self.compute_reward(self.state, self.goal)
        self.capture_frame()
        return self.state.clone(), reward, done

    def capture_frame(self):
        """ Store the raw state and goal as a NumPy array for animation. """
        self.frames.append(np.vstack([self.state.numpy(), self.goal.numpy()]))

    def render(self):
        """ Display the current state and goal side by side as a grid. """
        fig, ax = plt.subplots(1, 2, figsize=(5, 2))
        
        ax[0].imshow(self.state.unsqueeze(0), cmap="RdYlGn", aspect="auto", vmin=0, vmax=1)
        ax[0].set_title("State")
        ax[0].axis("off")

        ax[1].imshow(self.goal.unsqueeze(0), cmap="RdYlGn", aspect="auto", vmin=0, vmax=1)
        ax[1].set_title("Goal")
        ax[1].axis("off")
    ...
    def show_video(self):
        """ Play an animation of the agent's trajectory. """
        fig, ax = plt.subplots(figsize=(6, 2))

        def update(frame):
            ax.clear()
            ax.imshow(frame, cmap="RdYlGn", aspect="auto", vmin=0, vmax=1)
            ax.set_xticks([])
            ax.set_yticks([0, 1])
            ax.set_yticklabels(["State", "Goal"])
            ax.set_title("Environment Rollout")

        ani = animation.FuncAnimation(fig, update, frames=self.frames, repeat=False)
        plt.show()

    def generate_video(self, filename="bitflip_rollout.mp4"):
        """ Save an animation of the agent's trajectory as a video. """
        fig, ax = plt.subplots(figsize=(6, 2))
...
def train(num_bits=10, num_epochs=10, hindsight_replay=True,
    ...
    for _ in range(num_bits):
        ...
        action = agent.take_action(state_, eps_max)
        state, _, done = env.step(action.item())
        if done:
            break
    env.generate_video("bitflip_initial.mp4")
    ...
    for _ in range(num_bits):
        state_ = torch.cat((state, goal))
        action = agent.take_action(state_, eps_min)
        state, _, done = env.step(action.item())
        if done:
            break
    env.generate_video("bitflip_final.mp4")
